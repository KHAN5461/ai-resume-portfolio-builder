...
const ResumePreview = React.memo(() => {
    ...
    return (
        <PreviewErrorBoundary>
            <div className={`shadow-lg h-full overflow-y-auto bg-gray-50 custom-scrollbar font-${(resumeInfo?.themeFont || 'sans').toLowerCase().replace(' ', '-')}`}>
                {/* Default to ClassicTemplate for all resumes */}
                <ClassicTemplate data={templateData} accentColor={accentColor} />
            </div>
        </PreviewErrorBoundary>
    )
})
